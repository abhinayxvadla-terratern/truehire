...
export const EmployerRmRevealGateTab: React.FC = () => {
  const { user } = useAuth();
  ...
  const fetchMyEmployersRevealApplications = async () => {
    if (!user) return;
    ...
    try {
      ...
      // 3. Fetch applications in offer_sent or reveal_gate for these jobs
      ...
      console.error('Error fetching employer RM reveal applications:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchMyEmployersRevealApplications();
  }, [user]);
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-slate-900 tracking-tight">
          Reveal Gate Coordination (My Employers)
        </h1>
        <p className="text-xs text-slate-500 mt-0.5">
          Verify contract generation, escrow deposits, and candidate consents for job offers extended by your assigned hospital employers.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#E2E8F4] rounded-[10px] py-16 px-4 text-center shadow-2xs">
          ...
          <h3 className="text-sm font-semibold text-slate-700">
            No offers currently awaiting Reveal Gate
          </h3>
          <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
            When candidates receive job offers from your assigned hospital partners, they will appear here for condition checks.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
...
export default EmployerRmRevealGateTab;
